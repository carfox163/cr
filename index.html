...
<body>
    ...
    <a href="http://2006b.ys168.com" target="_blank">
        <div class="header">by Evita & CreativeLearner v2.0</div>
    </a>
    <!-- 将图片包裹在 <a> 标签内 -->
    <a href="http://2006b.ys168.com" target="_blank">
        <img src="http://ys-j.ysepan.com/wap/2006b/1if6a7FNIF6b9B7ISB/Ac4fD9jq3HChGQa4nfajDHD2gaNKc/CreativeLearner%203.png" width="182"
            height="110" style="position: fixed; top: 55px; right: 10px;">
    </a>
    ...
    <div class="game-container" id="gameContainer">
        <div class="group" id="groupA">
            ...
            <div class="word-box" id="wordA"></div>
        ...
        <div class="group" id="groupB">
            ...
            <div class="word-box" id="wordB"></div>
        ...
        <div class="group" id="groupC" style="display: none;">
            ...
            <div class="word-box" id="wordC"></div>
    ...
    <script>
        ...
        function updateGameTime() {
            ...
            if (!isNaN(newTime) && newTime >= 10 && newTime <= 600) {
                timeLeft = newTime;
                document.getElementById('timer').textContent = `计时：${timeLeft}`;
            }
        }
        ...
        function toggleGroupsCount() {
            ...
            if (groupsCount < 3) {
                ...
                document.getElementById('remainingC').textContent = wordsData.length;
                document.getElementById('resultC').innerHTML = '';
            }
            if (groupsCount < 2) {
                usedWords.B = [];
                results.B = { correct: 0, wrong: 0 };
                document.getElementById('remainingB').textContent = wordsData.length;
            ...
            if (groupsCount === 1) {
                ...
                document.getElementById('groupA').style.width = '70%';
            } else {
                gameContainer.style.justifyContent = 'space-around';
                document.getElementById('groupA').style.width = '45%';
                document.getElementById('groupB').style.width = '45%';
                document.getElementById('groupC').style.width = '45%';
            }
        }
        ...
        function saveResults() {
            ...
            // 根据用户地区设置分隔符
            const separator = navigator.language === 'de-DE'? ';' : ',';
        ...
        function updateRemainingWords() {
            document.getElementById('remainingA').textContent = wordsData.length - usedWords.A.length;
            document.getElementById('remainingB').textContent = wordsData.length - usedWords.B.length;
            document.getElementById('remainingC').textContent = wordsData.length - usedWords.C.length;
        }

        function prevQuestion(group) {
            const usedWordIndex = usedWords[group].indexOf(currentWords[group]);
            if (usedWordIndex > 0) {
                const prevWord = usedWords[group][usedWordIndex - 1];
                currentWords[group] = prevWord;
                displayQuestion(group, prevWord);

                // 重置该题的结果显示
                const resultElement = document.getElementById(`result${group}`);
        ...
        function nextQuestion(group) {
            ...
            if (availableWords.length > 0) {
                const randomWord = availableWords[Math.floor(Math.random() * availableWords.length)];
                currentWords[group] = randomWord;
                usedWords[group].push(randomWord);
                displayQuestion(group, randomWord);
                updateRemainingWords();
        ...
        function displayQuestion(group, word) {
            setupQuestion(group, word);
        }
